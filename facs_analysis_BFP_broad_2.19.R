...
dir="/Users/benjaminchang/Desktop/Collins Lab/Flow Files/Exp_20230217_2/Samples"
...
g.live <- polygonGate(filterId = "Live","FSC.A"=c(coor1[1],coor2[1],coor3[1],coor4[1]),"SSC.A"=c(coor1[2],coor2[2],coor3[2],coor4[2]))
...
# g.bfp <- rectangleGate(filterId="BFP_pos",BFP=c(800,60000))
# # check gate
# gs_pop_add(gs,g.bfp,parent="Live") # add gate to GatingSet
# recompute(gs) # recalculate Gatingset
# ggcyto(gs,aes(x=BFP),subset="Live",)+geom_density(fill="forestgreen")+geom_gate("BFP_pos")+ geom_stats(adjust = 0.1,y=0.002,digits = 1)+scale_x_flowjo_biexp()+facet_wrap(~name,ncol = 8)
# #save plot
